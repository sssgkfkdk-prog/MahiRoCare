  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (identifier === 'MahiRoCareAdmin' && password === 'MahiRoCare') {
          return { id: 'admin-hardcoded', email: 'admin@mahirocare.com', name: 'Super Admin', role: 'admin' };
        }

        await dbConnect();

        // Search by both email and name (username)
  ...
  callbacks: {
    async signIn({ user, account }) {
      if (account?.provider === 'google') {
        ...
        // Explicitly attach database ID and role to the NextAuth user object
